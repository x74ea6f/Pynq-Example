...
typedef struct {
    ...
    XCopy_mem_uio_map maps[ MAX_UIO_MAPS ];
} XCopy_mem_uio_info;

/***************** Variable Definitions **************************************/
static XCopy_mem_uio_info uio_info;
...
static int uio_info_read_name(XCopy_mem_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCopy_mem_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCopy_mem_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCopy_mem_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCopy_mem_Initialize(XCopy_mem *InstancePtr, const char* InstanceName) {
	XCopy_mem_uio_info *InfoPtr = &uio_info;
